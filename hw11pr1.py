...
scene.width = 800                      # Make the 3D canvas larger
scene.height = 560
...
def make_alien(starting_position, starting_vel = vector(0, 0, 0)):
    ...
    alien_eye3 = sphere(size = 0.3*vector(1, 1, 1), pos = .42*vector(.6, .2, .7), color = color.white)
...
def make_alien3(starting_position, starting_vel = vector(0, 0, 0)):
    """The lines below make a new "frame", which is a container with a
       local coordinate system.
       The arguments to make_alien allow for any initial starting position
       and initial starting velocity, with a default starting velocity
       ...
       alien's components:
    """
    alien_body = cylinder(size = 1.0*vector(1, 1, 1), pos = vector(0, 0, 0), color = color.green)
    alien_eye1 = sphere(size = 0.3*vector(1, 1, 1), pos = .42*vector(.7, .7, .7), color = color.red)
    alien_eye2 = sphere(size = 0.3*vector(1, 1, 1), pos = .42*vector(0, .7, .7), color = color.red)
    alien_eye3 = sphere(size = 0.3*vector(1, 1, 1), pos = .42*vector(1.5, .7, .7), color = color.red)
    alien_hat = cylinder(pos = 0.42*vector(.2, 0.9, -.2), axis = vector(.02, .2, -.02), size = vector(0.2, 0.7, 0.7), color = color.magenta)
    alien_objects = [alien_body, alien_eye1, alien_eye2, alien_eye3, alien_hat]  # make a list to "fuse" with a compound
    # now, we create a compound -- we'll name it com_alien:
    com_alien = compound(alien_objects, pos = starting_position)
    com_alien.vel = starting_vel   # set the initial velocity
...
wallD = box(pos = vector(10, 0, 0), axis = vector(0, 0, 1), size = vector(20,1, .2), color = color.red) # red
...
# Create two cylinders moving across the field
rod = cylinder(pos = vector(2, 0, 3), axis = vector(1, 0, 0), size = vector(1.5, 1.5, 1.5), color = color.yellow)
rod2 = cylinder(pos = vector(-5, 0, -5), axis = vector(1, 0, 0), size = vector(1.5, 1.5, 1.5), color = color.yellow)
# We make two aliens using two calls to the make_alien function (from above)
alien = make_alien(starting_position = vector(6, 0, -6), starting_vel = vector(0, 0, -3))
alien2 = make_alien(starting_position = vector(-10, 5, -10))  # zero starting velocity

alien_new = make_alien(starting_position = vector(-6, 0, 6), starting_vel = vector(5, 0, 0))
alien3 = make_alien3(starting_position = vector(-4, 0, -3), starting_vel = vector(2, 0, 3))
...
while True:
    ...
    alien_new.pos = alien_new.pos + alien_new.vel*dt
    ...
    corral_collide(alien_new)
    corral_collide(alien3)

    rod_collide(ball)


    # If the ball collides with the aliens, give the alien a vertical velocity
    if mag(ball.pos - alien.pos) < 1.0:
        print("To infinity and beyond!")
        alien.color = color.gray(.8)
        alien.vel = vector(0, 1, 0)

    if mag(ball.pos - alien_new.pos) < 1.0:
        print("To infinity and beyond!")
        alien_new.color = color.gray(.8)
        alien_new.vel = vector(0, 1, 0)
...
def rod_collide(ball):
    """Cylinder Collisions!
       Ball hits the cylinder, you must return to the starting position
    """

    # If the ball hits the rod
    if ball.pos.x == rod.pos.x:
       ball.vel.x = -1.0

    if ball.pos.z == rod.pos.z:
        ball.vel.z = -1.0

    if ball.pos.x == rod2.pos.x:
       ball.vel.x = -1.0

    if ball.pos.z == rod2.pos.z:
        ball.vel.z = -1.0
